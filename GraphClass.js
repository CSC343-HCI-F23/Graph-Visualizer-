export default class GraphClass {
    ...
    genHash() {
      ...
      */
        let s =
            JSON.stringify(this.graph.nodes.map((n) => n.id)) +
            JSON.stringify(
                this.graph.edges.map((e) => e.source.id + e.target.id)
            );
    ...
    extractLargestComponent() {
      ...
      */
        ...
        let subNodes = this.graph.nodes.filter((n) =>
            component.has(indexMap.get(n.id))
        );
        let subEdges = this.graph.edges.filter(
            (e) =>
                component.has(indexMap.get(e.source)) &&
                component.has(indexMap.get(e.target))
        );
    ...
    bfsForShortestPaths(sourceNode, graph) {
        ...
        while (queue.length > 0) {
            ...
            for (const edge of graph.edges) {
                ...
                if (edgeSource === current.id && !visited.has(edgeTarget)) {
                    visited.add(edgeTarget);
                    distances[edgeTarget] = distances[current.id] + 1;
                    queue.push(
                        graph.nodes.find((node) => node.id === edgeTarget)
                    );
                } else if (
                    ...
                    distances[edgeSource] = distances[current.id] + 1;
                    queue.push(
                        graph.nodes.find((node) => node.id === edgeSource)
                    );
                }
            }
        }
    ...
    computeAPL() {
        ...
        for (const node of largestComponent.nodes) {
            const shortestPaths = this.bfsForShortestPaths(
                node,
                largestComponent
            );
            sum += Object.values(shortestPaths).reduce((a, b) => a + b, 0);
        }

        return (
            sum /
            (largestComponent.nodes.length *
                (largestComponent.nodes.length - 1))
        );
    }
}
